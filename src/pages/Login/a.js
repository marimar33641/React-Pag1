...
function Formulario() {
  ...
  const handleProductoChange = (index, field, value) => {
    ...
    setProductoActual({ ...productoActual, cantidad: parseInt(value) });
  
    const suma = newProductos.reduce((total, producto) => {
      return total + parseInt(producto.cantidad);
    }, 0);
  
    const valorCompra = newProductos.reduce((total, producto) => {
      const productoData = productosData.message.find((p) => p.nombre === producto.nombre);
      return total + parseInt(producto.cantidad) * parseFloat(productoData.precio);
    }, 0);
  
    const puntosGanados = Math.floor(valorCompra / 10000);
